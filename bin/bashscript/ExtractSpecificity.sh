...
if [[ ${READS} == "true" ]]; then
	...
	if [[ -e $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.ontarget.gz && $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.offtarget.gz ]]; then
		...
		if [[ -e $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.ontarget && -e $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.offtarget ]]; then
			singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY awk -v name=${SAMPLEID} '{if(FILENAME~/offtarget/){offT+=$3} else if(FILENAME~/ontarget/){onT+=$3}} END{print name"\t"onT/(onT+offT)}' $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.o* >> $ROOT_PATH/Specificity/allSamples.ampregion.specificity;
			# gzip the files again to save space
			singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.ontarget;
			singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.offtarget;
		else
	...
	if [[ -e $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.ontarget.gz && $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.offtarget.gz ]]; then
		...
		if [[ -e $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.ontarget && -e $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.offtarget ]]; then
			singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY awk -v name=${SAMPLEID} '{if(FILENAME~/offtarget/){offT+=$3} else if(FILENAME~/ontarget/){onT+=$3}} END{print name"\t"onT/(onT+offT)}' $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.o* >> $ROOT_PATH/Specificity/allSamples.seqregion.specificity;
	        # gzip the files to save space
	        singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.ontarget;
	        singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.offtarget;
	    else
