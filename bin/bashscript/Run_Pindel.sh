...
if [[ ! -e $ROOT_PATH/PindelOut/${SAMPLEID}.indels_TD || ! -z $FORCE ]]; then
	if [[ ${NORMAL_SAMPLEID} == "annovar" ]]; then
		...
		if [[ -e $ROOT_PATH/Bwa/${SAMPLEID}.sorted.bam ]]; then
			...
			singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY pindel -f ${GENOME_FASTA_REF} -i $ROOT_PATH/PindelOut/${SAMPLEID}.config.txt -J $SERA_PATH/res/excludeChrM.txt -o $ROOT_PATH/PindelOut/${SAMPLEID}.indels ${PINDEL_FLAGS}
		else
		...
		if [[ -e $ROOT_PATH/Bwa/${SAMPLEID}.sorted.bam && $ROOT_PATH/Bwa/${NORMAL_SAMPLEID}.sorted.bam ]]; then
			...
			singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY pindel -f ${GENOME_FASTA_REF} -i $ROOT_PATH/PindelOut/${SAMPLEID}.config.txt -c ALL  -o $ROOT_PATH/PindelOut/${SAMPLEID}.indels ${PINDEL_FLAGS}
		else
