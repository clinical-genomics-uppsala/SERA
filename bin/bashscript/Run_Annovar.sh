	...
	if [[ ${READS} == "true" ]]; then
			if [[ ${NORMAL_SAMPLEID} == "annovar" ]]; then
				if [[ -e $ROOT_PATH/Annovar/${SAMPLEID}.annovarInput ]]; then
					singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $ROOT_PATH_ANNOVAR/table_annovar.pl $ROOT_PATH/Annovar/${SAMPLEID}.annovarInput $ANNOVAR_MODULES/annovar_humandb/ -protocol refGene,1000g2015aug_eur,snp138,snp138NonFlagged,esp6500siv2_ea,cosmic70,clinvar_20150629 -operation g,f,f,f,f,f,f -nastring "-" -otherinfo -buildver hg19 -remove -arg '-splicing_threshold 5',,,,,,
					singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $SERA_PATH/bin/perlscript/createAnnovarOutput.pl -i $ROOT_PATH/Annovar/${SAMPLEID}.annovarInput.hg19_multianno.txt -o $ROOT_PATH/AnnovarOutput/${SAMPLEID}.singleSample.annovarOutput -s
					...
					singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $SERA_PATH/bin/perlscript/createAnnovarOutput.pl -i $ROOT_PATH/Annovar/${SAMPLEID}.ampliconmapped.annovarInput.hg19_multianno.txt -o $ROOT_PATH/AnnovarOutput/${SAMPLEID}.singleSample.ampliconmapped.annovarOutput -s -am
				...
				if [[ -e $ROOT_PATH/Annovar/${SAMPLEID}_${NORMAL_SAMPLEID}.annovarInput ]]; then
					...
					singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $SERA_PATH/bin/perlscript/createAnnovarOutput.pl -i $ROOT_PATH/Annovar/${SAMPLEID}_${NORMAL_SAMPLEID}.annovarInput.hg19_multianno.txt -o $ROOT_PATH/AnnovarOutput/${SAMPLEID}_${NORMAL_SAMPLEID}.tumorNormalSample.annovarOutput -tn
					...
					singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $SERA_PATH/bin/perlscript/createAnnovarOutput.pl -i $ROOT_PATH/Annovar/${SAMPLEID}_${NORMAL_SAMPLEID}.ampliconmapped.annovarInput.hg19_multianno.txt -o $ROOT_PATH/AnnovarOutput/${SAMPLEID}_${NORMAL_SAMPLEID}.tumorNormalSample.ampliconmapped.annovarOutput -tn -am
