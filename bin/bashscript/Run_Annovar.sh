	...
	if [[ ${READS} == "true" ]]; then
			if [[ ${NORMAL_SAMPLEID} == "annovar" ]]; then
				if [[ -e $ROOT_PATH/Annovar/${SAMPLEID}.annovarInput ]]; then
					...
					singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $SERA_PATH/bin/perlscript/createAnnovarOutput.pl -i $ROOT_PATH/Annovar/${SAMPLEID}.annovarInput.hg19_multianno.txt -o $ROOT_PATH/AnnovarOutput/${SAMPLEID}.singleSample.annovarOutput -s
					...
					singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY perl $SERA_PATH/bin/perlscript/createAnnovarOutput.pl -i $ROOT_PATH/Annovar/${SAMPLEID}.ampliconmapped.annovarInput.hg19_multianno.txt -o $ROOT_PATH/AnnovarOutput/${SAMPLEID}.singleSample.ampliconmapped.annovarOutput -s -am
