	...
	if [[ ${READS} == "true" ]]; then
		if [[ ${DESIGN_TYPE} == "PCR" ]]; then
			if [[ -e $ROOT_PATH/SNPmania/${SAMPLEID}.ampliconmapped.variations ]]; then
				singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY python2.7 $SERA_PATH/bin/pythonscript/calculateGeneCov_bed.py -i ${ROIFILE} -v $ROOT_PATH/SNPmania/${SAMPLEID}.ampliconmapped.variations -o /dev/stdout -g | sort -k3,4 > $ROOT_PATH/GeneCov/${SAMPLEID}.geneCov.txt
			elif [[ -e $ROOT_PATH/SNPmania/${SAMPLEID}.variations ]]; then
				singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY python2.7 $SERA_PATH/bin/pythonscript/calculateGeneCov_bed.py -i ${ROIFILE }-v $ROOT_PATH/SNPmania/${SAMPLEID}.variations -o /dev/stdout | sort -k3,4 > $ROOT_PATH/GeneCov/${SAMPLEID}.geneCov.txt
			else
