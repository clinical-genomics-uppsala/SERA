	...
	if [[ ${READS} == "true" ]]; then
		...
		if [[ -e $ROOT_PATH/Pileup/${SAMPLEID}.pileup.gz ]]; then
			...
			if [[ -e $ROOT_PATH/refFiles/${REFSEQ}.ampregion ]]; then
				...
				singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip -f $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.ontarget;
				singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip -f $ROOT_PATH/Specificity/${SAMPLEID}.ampregion.offtarget;
			else
			...
			if [[ -e $ROOT_PATH/refFiles/${REFSEQ}.seqregion ]]; then
				...
				# gzip output files
				singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip -f $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.ontarget;
				singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gzip -f $ROOT_PATH/Specificity/${SAMPLEID}.seqregion.offtarget;
			else
