    ...
    if [[ "$MATE_PAIR" == "true" ]]; then
        ...
        for chrom in ${StringArray[@]}; do
            if [[ ${METHOD} == "haloplex" ]]; then
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "gatk --java-options '-Xmx4g' Mutect2 -L chr$chrom -I $ROOT_PATH/Bwa/${SAMPLEID}.sorted.bam -bamout $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.bam -R /data/ref_genomes/hg19/genome_fasta/hg19.with.mt.fasta -O $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.raw.vcf -DF NotDuplicateReadFilter --disable-adaptive-pruning" && singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH /projects/wp2/nobackup/Twist_Myeloid/Containers/gatk4.1.4.1.simg sh -c "java -Xmx12g -jar /gatk/gatk-package-4.1.4.1-local.jar FilterMutectCalls -R /data/ref_genomes/hg19/genome_fasta/hg19.with.mt.fasta -V $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.raw.vcf -O $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.vcf &&  bgzip $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.vcf && tabix $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.vcf.gz"
           elif [[ ${METHOD} == "swift" ]]; then
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "gatk --java-options '-Xmx4g' Mutect2 -L chr$chrom -I $ROOT_PATH/Bwa/${SAMPLEID}.sorted.bam -bamout $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.bam -R /data/ref_genomes/hg19/genome_fasta/hg19.with.mt.fasta -O $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.raw.vcf" && singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH /projects/wp2/nobackup/Twist_Myeloid/Containers/gatk4.1.4.1.simg sh -c "java -Xmx12g -jar /gatk/gatk-package-4.1.4.1-local.jar FilterMutectCalls -R /data/ref_genomes/hg19/genome_fasta/hg19.with.mt.fasta -V $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.raw.vcf -O $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.vcf &&  bgzip $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.vcf && tabix $ROOT_PATH/Mutect2/${SAMPLEID}.sorted.${chrom}.vcf.gz"
           else
