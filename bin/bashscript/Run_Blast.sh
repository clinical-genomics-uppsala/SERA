...
if [[ ${READS} == "true" ]]; then 
	...
	zcat $ROOT_PATH/BLAST/Runs/${SAMPLEID}.blast.gz | awk -v ml=${BLAST_ML} -v mm=${BLAST_MM} '{if(($4>=ml)&&($5<=mm)&&($6==0)&&($9==1)) {print $1;}}' | sort | uniq -c | awk '{print $2 "\t" $1;}' | sort -k2nr | gzip - > $ROOT_PATH/BLAST/Runs/${SAMPLEID}.blast.filtered.gz;
