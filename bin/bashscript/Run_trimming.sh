    ...
    if [[ "$MATE_PAIR" == "true" ]]; then
        ...
        if [[ ${METHOD} == "haloplex" ]]; then
            ...
            if [[ ! -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.tmp.fastq.gz && ! -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read2.tmp.fastq.gz || ! -z $FORCE ]]; then
                ...
                PE1_G_T="${ROOT_PATH}/seqdata/${SAMPLEID}.read1.fastq.gz";
                PE2_G_T="${ROOT_PATH}/seqdata/${SAMPLEID}.read2.fastq.gz";

                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY java -Xmx32g -Xms16g -jar ${TRIMMOMATIC_JAR} PE \
                    -threads 8 -trimlog $TRIM_LOG \
            ...
            if [[ ! -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.tmp.fastq.gz && ! -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read2.tmp.fastq.gz || ! -z $FORCE ]]; then
                ...
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gunzip -f ${PE1_G_T};
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY gunzip -f ${PE2_G_T};
                PE1_T="${PREFIX}_R1_trimd.fq";
                PE2_T="${PREFIX}_R2_trimd.fq";

                # convert fastq format to one line per record for splitting
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "paste - - - - < $PE1_T | tr '\t' '~' > ${PE1_T}.tmp1";
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "paste - - - - < $PE2_T | tr '\t' '~' > ${PE2_T}.tmp1";
                ...
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY split -d -l $chunklinecnt ${PE1_T}.tmp1 ${PREFIX}_r1split
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY split -d -l $chunklinecnt ${PE2_T}.tmp1 ${PREFIX}_r2split
                ...
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY parallel r1reformat ::: ${PREFIX}_r1split*
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY parallel r2reformat ::: ${PREFIX}_r2split*
                ...
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY parallel --xapply ptrim {1} {2} ::: $(cat ${PREFIX}_r1infiles) ::: $(cat ${PREFIX}_r2infiles)

                # concatenate primer-trimmed fastq chunks
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "cat ${PREFIX}_*_R1_primertrimd.fq | gzip -f > ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.fastq.gz";
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "cat ${PREFIX}_*_R2_primertrimd.fq | gzip -f > ${ROOT_PATH}/seqdata/${SAMPLEID}.read2.fastq.gz";
...
if [[ "$?" != "0" ]]; then
    ErrorLog "${SAMPLEID}" "Failed in cutadapt (haloplex and swift) or trimmomatic (swift+breast)...";
else
