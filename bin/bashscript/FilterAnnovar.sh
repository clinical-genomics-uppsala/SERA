                ...
                if [[ -e $ANNOVARFILE && $PINDELANNOVARFILE ]]; then
                    if [[ ${TISSUE} == "ovarial" || ${TISSUE} == "breast" ]]; then
                        ...
                        if [[ $KEEPFILE == "false" ]]; then
                            ...
                            echo "cat  $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.02 -a"
                            ...
                            singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "cat  $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.02 -k $KEEPFILE -a"
                            echo "cat  $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.02 -k $KEEPFILE -a"
                        fi
                    else
                        # Check if there are particular regions we want to keep
                        if [[ $KEEPFILE == "false" ]]; then
                            singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "cat $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.01 -a"
                            echo "cat $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.01 -a"
                        else
                            singularity exec -B /data -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY sh -c "cat $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.01 -k $KEEPFILE -a"
                            echo "cat $ANNOVARFILE $PINDELANNOVARFILE | python2.7 $SERA_PATH/bin/pythonscript/FilterAnnovarOutput.py -i /dev/stdin -o $ROOT_PATH/FilteredAnnovarOutput/${SAMPLEID}.singleSample.filtered.annovarOutput -b $BLACKLIST_FILE -g 0.01 -k $KEEPFILE -a"
                        fi
                    fi
                else
