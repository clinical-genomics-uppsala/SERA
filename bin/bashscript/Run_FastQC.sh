...
if [[ $PLATFORM = "Illumina" ]]; then
    ...
    if [[ "$MATE_PAIR" == "true" ]]; then
        ...
        if [[ -e ${READ1} && ${READ1} ]]; then
            singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY fastqc -o FastQC ${READ1} ${READ1} > $ROOT_PATH/FastQC/fastqc_${SAMPLEID}_output.txt;
        else
        ...
        if [[ ! -e $ROOT_PATH/FastQC/${SAMPLEID}.read1_fastqc.html || ! -e $ROOT_PATH/FastQC/${SAMPLEID}.read2_fastqc.html || ! -z $FORCE ]]; then
            ...
            if [[ -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.fastq.gz && -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read2.fastq.gz ]]; then
                singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY fastqc -o FastQC ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.fastq.gz ${ROOT_PATH}/seqdata/${SAMPLEID}.read2.fastq.gz > $ROOT_PATH/FastQC/fastqc_${SAMPLEID}_outputCutadapt.txt;
            else
    ...
    # If single reads are used only care about the first read
        ...
        if [[ -e ${READ2} ]]; then
            singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY fastqc -o FastQC ${READ2} > $ROOT_PATH/FastQC/fastqc_${SAMPLEID}_output.txt;
        else
        ...
        if [[ -e ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.fastq.gz ]]; then
            singularity exec -B /data -B /scratch -B /opt -B /beegfs-storage -B /projects -B $SERA_PATH $SERA_SINGULARITY fastqc -o FastQC ${ROOT_PATH}/seqdata/${SAMPLEID}.read1.fastq.gz > $ROOT_PATH/FastQC/fastqc_${SAMPLEID}_outputCutadapt.txt;
        else
