                    ...
                    if [[ (! -e $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv) || ($FORCE == "true") ]]; then
                        if [[ ${TYPE} == "ffpe" ]]; then
                            if [[ ${TISSUE} == "ovarial" || ${TISSUE} == "breast" ]]; then
                                echo "cat $ANNOVARFILE $PINDELANNOVARFILE | python $SERA_PATH/bin/pythonscript/FilterAllMutations_indel.py -i ${HOTSPOTFILE} -o $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv -f /dev/stdin -b $BLACKLIST_FILE -s ${SAMPLEID} -chr2nc $NC2chr -g 0.02 ${ampMapped} -t ${MAIN_TRANSCRIPTS} -v ${SNPMANIAFILE} -m $ROOT_PATH/refFiles/MultipleBp_variations.csv ${MUTATION_FLAGS}"
                                cat $ANNOVARFILE $PINDELANNOVARFILE | python $SERA_PATH/bin/pythonscript/FilterAllMutations_indel.py -i ${HOTSPOTFILE} -o $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv -f /dev/stdin -b $BLACKLIST_FILE -s ${SAMPLEID} -chr2nc $NC2chr -g 0.02 ${ampMapped} -t ${MAIN_TRANSCRIPTS} -v ${SNPMANIAFILE} -m $ROOT_PATH/refFiles/MultipleBp_variations.csv ${MUTATION_FLAGS}
                            else
                                echo "cat $ANNOVARFILE $PINDELANNOVARFILE | python $SERA_PATH/bin/pythonscript/FilterAllMutations_indel.py -i ${HOTSPOTFILE} -o $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv -f /dev/stdin -b $BLACKLIST_FILE -s ${SAMPLEID} -chr2nc $NC2chr -g 0.01 ${ampMapped} -t ${MAIN_TRANSCRIPTS} -v ${SNPMANIAFILE} -m $ROOT_PATH/refFiles/MultipleBp_variations.csv ${MUTATION_FLAGS}"
                                cat $ANNOVARFILE $PINDELANNOVARFILE | python $SERA_PATH/bin/pythonscript/FilterAllMutations_indel.py -i ${HOTSPOTFILE} -o $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv -f /dev/stdin -b $BLACKLIST_FILE -s ${SAMPLEID} -chr2nc $NC2chr -g 0.01 ${ampMapped} -t ${MAIN_TRANSCRIPTS} -v ${SNPMANIAFILE} -m $ROOT_PATH/refFiles/MultipleBp_variations.csv ${MUTATION_FLAGS}
                            fi
                            ...
                            if [[ ${TISSUE} == "ovarial" || ${TISSUE} == "breast" ]]; then
                                ...
                                echo "cat $ANNOVARFILE $PINDELANNOVARFILE | python $SERA_PATH/bin/pythonscript/FilterAllMutations_indel.py -i ${HOTSPOTFILE} -o $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv -f /dev/stdin -b $BLACKLIST_FILE -s ${SAMPLEID} -chr2nc $NC2chr -g 0.01 ${ampMapped} -t ${MAIN_TRANSCRIPTS} -v ${SNPMANIAFILE} -m $ROOT_PATH/refFiles/MultipleBp_variations.csv ${MUTATION_PLASMA_FLAGS}"
                                cat $ANNOVARFILE $PINDELANNOVARFILE | python $SERA_PATH/bin/pythonscript/FilterAllMutations_indel.py -i ${HOTSPOTFILE} -o $ROOT_PATH/FilteredMutations/${SAMPLEID}.filteredMutations.tsv -f /dev/stdin -b $BLACKLIST_FILE -s ${SAMPLEID} -chr2nc $NC2chr -g 0.01 ${ampMapped} -t ${MAIN_TRANSCRIPTS} -v ${SNPMANIAFILE} -m $ROOT_PATH/refFiles/MultipleBp_variations.csv ${MUTATION_PLASMA_FLAGS}
                            fi
                        else
