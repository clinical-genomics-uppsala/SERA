...
Edited by Claes Ladenvall 20150528
To handle files deposited by the SNP & SEQ platform for the second round of Haloplex run

This input file generator had to be modified because the output generated from SNP&SEQ look different compared to the first run
This version will take all samples from lane 003.
...
with (open(output, mode = 'w'))as outfile:
    ...
    for sample in infoSort:
        ...
        outfile.write ("RAWDATA_PE1_ARR_[${COUNT}]=\"$RAW_PATH/" + sample + "_" + info[sample]['barcode'] + "_L003_R1_001.fastq.gz" + "\";\n")
        outfile.write ("RAWDATA_PE2_ARR_[${COUNT}]=\"$RAW_PATH/" + sample + "_" + info[sample]['barcode'] + "_L003_R2_001.fastq.gz" + "\";\n")
