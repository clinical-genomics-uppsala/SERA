...
with open(args.filteredAnnovarfile, 'r') as filteredAnnovar:
    ...
    for line in filteredAnnovar:
        ...
        if not re.match('^#', line) and not re.match('^$', line) and not line.startswith('Sample'):
            ...
            # Add all known mutations to the mutation dictionary
            if not chrom in mutations:
                ...
                # Check if the info comes from pindel or annovar
                if ("+" in lineSplit[24] and "|" in lineSplit[24] and "-" in lineSplit[24]) or ("+" in lineSplit[25] and "|" in lineSplit[25] and "-" in lineSplit[25]):
                    mutations[chrom][start][ref][var]['from'] = "pindel"
                else:
                    mutations[chrom][start][ref][var]['from'] = "annovar"
            else:
                if not start in mutations[chrom]:
                    ...
                    if not ref in mutations[chrom][start]:
                        ...
                        if not var in mutations[chrom][start][ref]:
                            ...
                            # Check if the data comes from Pindel
                            if ("+" in lineSplit[24] and "|" in lineSplit[24] and "-" in lineSplit[24]) or ("+" in lineSplit[25] and "|" in lineSplit[25] and "-" in lineSplit[25]):
                                 # Check if the existing line is from annovar - if so change to pindel info
                                 if "annovar" in mutations[chrom][start][ref][var]['from']:
                                     mutations[chrom][start][ref][var]['from'] = "pindel"
                                     mutations[chrom][start][ref][var]['line'] = lineSplit
