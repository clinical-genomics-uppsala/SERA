...
my ( $next_arg, $variantFile, $insertionFile, $deletionFile, $outputFile, $nc2chr, $nc_col, $chr_col, $minRD, $minVariantAlleleRatio, $ampliconMapped );
        ...
        if ( $variant ne $line->[5] ) {
            ...
            if ( ( $alleles[$count] / $line->[0] ) >= $minVariantAlleleRatio ) {
                ...
                if ($ampliconMapped) {
                    ...
                    my $ref_amp_minus = $ref{ $line->[3] }{ $line->[4] }{'amp-'};
                    my $ref_amp_info  = $ref{ $line->[3] }{ $line->[4] }{'ampInfo'};
                ...
                if ($ampliconMapped) {
                    ...
                    foreach my $am (@ampliconOptions) {
                        ...
                        if ( $am =~ m/$match/ ) {
                            ...
                            # Count the number of plus and minus amplicons for the variant
                            ( $amp_plus, $amp_minus ) = countAmplicons( $am, "ins" );
                            $amp = $am;
...
sub getDeletions {
    ...
    if ( $line->[5] =~ m/\(/ ) {
        ...
        for ( my $i = 0 ; $i < scalar(@deletions) ; $i++ ) {
            ...
            if ( ( $deleteRD / $line->[0] ) >= $minVariantAlleleRatio && ( $deleteRD + $refRD ) >= $minRD ) {
                    ...
                    if ($ampliconMapped) {
                        ...
                        foreach my $am (@ampliconOptions) {
                            ...
                            if ( $am =~ m/$match/ ) {
                                ...
                                # Count the number of plus and minus amplicons for the variant
                                ( $amp_plus, $amp_minus ) = countAmplicons( $am, "del" );
                                $amp = $am;
...
sub checkMultipleBpVariant {
    ...
    for my $chrom (keys %varHash) {
        ...
        for my $p (sort {$a<=>$b} keys %{$varHash{$chrom}}){
            if ($lastPos == 0){ # If lastPos is unset, set it to this position
                ...
                if ($p == ($lastPos+1)) {
                    ...
                    # TODO do what you like to do with the things that are in the hash before you reset it
                    
                    my $ref = my $var  ="";
                    if (keys (%multipleBp) > 1){ # the variant consists of more than one bp
                        my @keyPos = sort {$a<=>$b} keys %multipleBp;
                        
                        my ($mainKey, $mainVaf) = getMajorVaf($multipleBp{$keyPos[0]});
                        my @line = split(/[\s=]/, $multipleBp{$keyPos[0]}{$mainKey});
                        ...
                        for (my $i = 0; $i<(scalar(@keyPos)-1); $i++) {
                            my ($mainKey, $mainVaf) = getMajorVaf($multipleBp{$keyPos[$i]});
                            my @line = split(/[\s=]/, $multipleBp{$keyPos[$i]}{$mainKey});
                            my ($ref, $var)=split("\#", $mainKey);
                            my $vaf = $line[9];
                            my $allelFreq = $line[11];
                            my $totRd = $line[13];
                            ...
                            for (my $j = $i+1; $j<scalar(@keyPos); $j++) {
                                my ($nextKey, $nextVaf) = getMajorVaf($multipleBp{$keyPos[$j]});
                                my @nextLine = split(/[\s=]/, $multipleBp{$keyPos[$j]}{$nextKey});
