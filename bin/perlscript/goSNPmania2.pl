...
# MAIN ---------
	...
	# Open files
	print "Processing goSNPmania result file ...\n";
		...
		if(!(m/^#/)) {
			...
			if (defined($snps->{$chr}->{$pos})) {
				...
				if(substr($snps->{$chr}->{$pos}->{'genotype'},0,1) eq substr($snps->{$chr}->{$pos}->{'genotype'},1,1)) {
					if(substr($snps->{$chr}->{$pos}->{'genotype'},0,1) eq "N") {
						$hetHom = -1;
					}
					else {
						$hetHom = 2;
...
sub usage {
  print "\nUsage: $0 -i <goSNPmania-file> -snp_ref <file,chr_col,pos_col,genotype_col> -o <outputfile>\n 
 -i  	   Input file goSNPmaina format.
 -snp_ref  SNP ref. file and columns with chromosome, position and genotype (separated by \",\".
           e.g. \"file.txt,2,3,4\".
 -o  	   Output file.
 -nc2chr   Try to convert NC-number to chr (e.g. NC_000019.8 -> chr19).
           chr23 and chr24 will be chrX and chrY.\n\n";   
  exit(1);
}
