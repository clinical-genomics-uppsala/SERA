...
pdf(pdffile, width=9, height=6);
	...
	# Main plot
	par(mar=c(5,5,3,1));
	plot(hapmap$V1, hapmap$V3, col=(hapmap$V15+1), pch=20, cex=0.6, ylim=c(0,1), xlim=c(20,max(hapmap$V1)), log="x", xlab="Sequencing Depth", ylab="Reference Allele Ratio", main=title);
	...
	for (i in 1:(NROW(hapmap))) {
	  if(hapmap$V1[i] >= readDepth) {
	     if(hapmap$V15[i] == 1) { heteroz[(hetCount=hetCount+1),] <- hapmap[i,]; }
	     if(hapmap$V15[i] == 2) { homoz[(homCount=homCount+1),] <- hapmap[i,]; }
	  }			
	}
