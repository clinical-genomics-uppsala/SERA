...
#par(mfrow=c(1,1))
par(mfrow=c(4,1), mar=(c(2,4,3,2)+0.1), oma=c(0,0,0,0))
...
#	mtitle <-strsplit(as.character(args[1]), "/")[[1]];
	...
	for (a in 1:nrow(ampregion)) {
		...
		selInAmp <- subset(RE_set, (ampregion[a,2]==RE_set[,2] & as.numeric(ampregion[a,3])<=as.numeric(RE_set[,3]) & as.numeric(ampregion[a,4])>=as.numeric(RE_set[,4])), select=c("fragID", "chr", "start", "end", "strand", "cnvRatio"));
