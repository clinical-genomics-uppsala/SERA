...
10 "Remove adapter sequences from reads" off "Run_cutAdapt_swift_all.sh" false 0 \
11 "Run FastQC" off "Run_FastQC.sh" "10" 0 \
12 "Align with BWA against Genome" off "Run_Bwa.sh" "10" 0 \
13 "Amplicon mapping" off "AmpliconMapping.sh" "12" 0 \
14 "Run jSNPmania" off "Run_jSNPmania.sh" "2:12:13" 0 \
15 "SNPmania output to Annovar input" off "jSNPmania2AnnovarInput.sh" "14" 0 \
16 "Run Annovar" off "Run_Annovar.sh" "15" 0 \
17 "Combine Annovar output to one file" off "Combine_annovarOutput_to_one_file.sh" "16" 2 \
20 "Run Pindel" off "Run_Pindel.sh" "12" 0 \
21 "Annotate Pindel with Annovar" off "AnnotatePindel.sh" "20" 0 \
25 "Output all mutations (hotspots and others)" off "FilterMutations.sh" "16:21" 0 \
26 "Convert Annovar output to vcf-format" off "ConvertAnnovaroutput2vcf.sh" "25" 0 \
30 "Extract MSI markers" off "ExtractMSI.sh" "16:21:25" 0 \
31 "Combine extracted MSI markers to one file" off "Combine_extracted_MSI.sh" "30" 2 \
32 "Extract EGFR information" off "Extract_T790M.sh" "14" 0 \
33 "Combine extracted EGFR information to one file" off "Combine_extracted_EGFR.sh" "32" 2 \
34 "Extract SNP information" off "ExtractSNPinfo.sh" "14" 0 \
35 "Combine extracted SNP information to one file" off "Combine_extracted_SNPs.sh" "34" 2 \
40 "Calculate amplification ratio" off "CalculateAmplificationRatio.sh" "14" 0 \
50 "Calculate fraction covered" off "FractionCovered.sh" "14" 0 \
51 "Calculate mean cov per gene" off "Calculate_Gene_Cov.sh" "14" 0 \
52 "Calculate mean cov per region (exon)" off "Calculate_Region_Cov.sh" "14" 0 \
60 "Create hits per base file from SNPmania file SeqRegion" off "SNPmania2hitsPerBase_seqregion.sh" "14" 0 \
61 "Create hits per base file from SNPmania file SeqRoi" off "SNPmania2hitsPerBase_seqroi.sh" "14" 0 \
...
100 "Copy results to OUTBOX" off "Move_results2outbox.sh" "16:21:25:26:31:33:35:40" 2 \
101 "Copy results to storage" off "Move_results2storage.sh" "16:21:25:26:31:33:35:40" 2);
