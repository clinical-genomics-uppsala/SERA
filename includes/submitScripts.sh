...
#				echo "Running post steps with dependency $DEP";	# TMP REMOVE ME!
		...
		SUBMIT_TEXT=$(sbatch -p core -J SERA-${SAMPLEID}-S${1} -e $ROOT_PATH/slurmOutput/${SAMPLEID}-S${1}.error -o $ROOT_PATH/slurmOutput/${SAMPLEID}-S${1}.output $SLURM_MISC_SETTINGS $DEP $SERA_PATH/bin/bashscript/${2});
